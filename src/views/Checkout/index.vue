...
import { getOrderInfoAPI, submitOrderToGetPayOrder } from '@/apis/modules/order'
...
const confirm = () => {
    ...
    curAddress.value = checkInfo.value.userAddresses.find((item) => item.id === activeAddressId.value)
}
    ...
    <el-dialog v-model="showDialog" title="切换收货地址" width="30%" center>
        <div class="addressWrapper">
            <div class="text item" v-for="item in checkInfo.userAddresses" :key="item.id"
                @click="handleChoseAddress(item.id)" :class="{ active: activeAddressId === item.id }">
                <ul>

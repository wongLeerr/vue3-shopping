...
支付逻辑：点击支付宝支付 -> 携带订单id和回跳地址（当支付状态生成后页面的跳转页面）到支付地址 （这个支付地址应该是商家和支付平台协商的地址，与前端无关，只知道要跳到这个地址下面付钱即可）-> （回跳动作是自动进行的，并且把必要的参数携带给我了，如支付状态等等，但注意人家帮我们跳转的路径是固定的/paycallback?payResult=true&orderId=1685107134624174082,我们只需配置路由，并设置相应页面即可）
从路由路径获取的参数基本上都是字符串（很少是纯数字和布尔值），这一点要清楚
封装倒计时组件（后端返回的是秒数，需将秒数转化为时分秒的倒计时形式,借助dayjs库实现），采用hook的形式

### 封装sku组件
sku组件的作用是为了让用户能够选择商品的价格，从而提交购物车，在选择的过程中，组件的选中状态要进行更新，组件还要提示用户当前规格是否禁用，每次选择都要产出对应的sku数据。

### 导航守卫
创建 store 实例需要放在beforeEach钩子中，因为如果在router/index.js顶部创建store实例，将发现pinia实例并没有注入到此文件之中
